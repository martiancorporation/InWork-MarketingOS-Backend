...
_CHECKS: tuple[_Check, ...] = (
    ...
    _Check("about", "About the brand", 8, lambda c: bool((c.about_brand or c.brand_extracted or "").strip())),
    ...
    _Check("contacts-client", "At least 1 client contact", 8, lambda c: _has_contact(c, ContactSide.client)),
    _Check("contacts-inwork", "At least 1 InWork contact", 8, lambda c: _has_contact(c, ContactSide.inwork)),
    _Check("integrations", "At least 1 integration connected", 12, lambda c: any(i.status == IntegrationStatus.connected for i in c.integrations)),
)
    ...
    def report(self, client: Client) -> ReadinessReport:
        ...
        for check in _CHECKS:
            if check.passes(client):
                ...
                missing.append(
                    ReadinessItem(key=check.key, label=check.label, weight=check.weight)
                )
