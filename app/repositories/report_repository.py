...
class ReportRepository(BaseRepository[Report]):
    ...
    def get_for_client(self, client_id: uuid.UUID, report_id: uuid.UUID) -> Report | None:
    ...
    def list_for_client(
        ...
        *,
        ...
        total = self.db.scalar(select(func.count()).select_from(Report).where(*conditions))
        stmt = select(Report).where(*conditions).order_by(Report.created_at.desc()).offset(offset)
